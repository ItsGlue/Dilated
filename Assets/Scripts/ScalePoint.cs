    ...
    void Update()
    {
        ...
        if (scaling) {
            ...
            if ((dist > 0 || player.GetComponent<PlayerMovement>().canOut) && (dist < 0 || player.GetComponent<PlayerMovement>().canIn)) {
                player.transform.Translate(-playerVec.normalized * Mathf.Clamp(dist * scaleFactor,-0.15f * Mathf.Abs(player.transform.localScale.y),0.15f * Mathf.Abs(player.transform.localScale.y)));
